...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
     ...
     */
    // things moddified for dvorak use
    //  H_SEM_COLON_BACK_TICK -> KC_SEMICOLON
    //  H_COLON_TILDE -> KC_GRAVE
    //   KC_LPRN,  KC_RPRN -> KC_LBRC, KC_RBRC
    //   KC_NUBS -> KC_BSLS # NUBS becomes "<>" otherwise
    //   H_L_BRACE__L_CHEVRON,  H_R_BRACE__R_CHEVRON -> ????
    [_BASE] = LAYOUT_combined(
        ...
        KC_F1,   KC_F2,    H_CLOSE,  H_OPEN,    KC_ESC,               KC_QUES,    KC_EXLM, UM(AT_SYMBOL),    UM(GBP), UM(EURO),    UM(JPY),  UM(BACKTICK),    UM(DQUOTE_OPEN), UM(DQUOTE_CLOSE), UM(PRIME), KC_UNDS,       KC_LABK, KC_RABK,  UM(DIVIDES), KC_LCBR,    KC_RCBR,  H_COMPLETE,     KC_CIRC,   KC_PERC,  KC_HASH,  KC_DLR,   //26

        KC_F3,   KC_F4,    KC_FIND,  H_WRITE,   QK_LEAD,  H_DOUBLE_QUOTE__PLUS_MINUS, KC_GRAVE,  KC_1,     KC_2,     KC_3,     KC_4,     KC_5,    KC_6,     KC_7,    KC_8,     KC_9,    KC_0,     KC_MINS,  KC_EQL,  KC_BSLS, H_L_BRACE__L_CHEVRON,  H_R_BRACE__R_CHEVRON,  KC_UNDO,             KC_TILD,   KC_SLSH,  KC_PAST,  KC_PMNS,  //27
        KC_F5,   KC_F6,    KC_EXLM,  KC_UNDO,   KC_PASTE, KC_CUT ,  KC_TAB ,  KC_Q,     KC_W,     KC_E,     KC_R,     KC_T,    KC_Y,     KC_U,    KC_I,     KC_O,    KC_P,     KC_LBRC,  KC_RBRC, KC_BSPC, KC_CLEAR, KC_HOME,                                                                    KC_P7,     KC_P8,    KC_P9,    KC_PPLS,  //26
        KC_F7,   KC_F8,    KC_SELECT,H_DEBUG,   MO(_FN), KC_NO,       MO(_APL), KC_A,     KC_S,     KC_D,     KC_F,     KC_G,    KC_H,     KC_J,    KC_K,     KC_L,    KC_SEMICOLON,      KC_QUOT,  KC_ENT , H_LINE, H_PAGE,                                                                     KC_P4,     KC_P5,    KC_P6,    KC_AMPR,  //25
        ...
        KC_F11,  KC_F12,   KC_HOME,  H_EOF,     KC_MPRV,  KC_MNXT,  H_7BIT,  KC_LALT,  KC_HYPR,  KC_LGUI,  KC_LCTL,  KC_SPC,  H_CIRCLE_SM,  KC_DEL,  KC_RCTL,  KC_RGUI, KC_HYPR,  KC_RALT,  KC_LEFT, KC_DOWN, KC_RGHT,                                                                           KC_DEL,    KC_P0,    KC_PDOT,  KC_ENTER, //25
                                                                                                                   KC_SPC
    ),
    ...
    [_APL] = LAYOUT_combined(
        ...
        FT,      FT,      FT,       FT,       FT,       FT,       FT,  UM(LOGICAL_AND), UM(LOGICAL_OR), UM(INTERSECT), UM(UNION), UM(SUBSET_OF),UM(SUPERSET_OF), UM(FORALL),UM(LEMNISCATE), UM(THERE_EXISTS),UM(PARTIAL_DIFFERENTIAL),      FT,       FT,      FT,      FT,       FT,                            FT,      FT,       FT,      FT,
        FT,      FT,      FT,       FT,       FT,       FT,       FT,  UM(UP_TACK), UM(DOWN_TACK), UM(RIGHT_TACK), UM(LEFT_TACK), UM(UPWARDS_ARROW),UM(DOWNARDS_ARROW), UM(LEFTWARDS_ARROW),UM(RIGHTWARDS_ARROW), UM(LEFTRIGHT_ARROW),     FT,       FT,       FT,      FT,      FT,                                      FT,      FT,       FT,      FT,
        FT,      FT,      FT,       FT,       FT,       FT,       FT,       FT,  UM(LEFT_FLOOR), UM(LEFT_CEILING), UM(NOT_EQUAL_TO), UM(ASYMPTOTICALLY_EQUAL_TO),UM(NOT_ASYMPTOTICALLY_EQUAL_TO), UM(LESS_THAN_OR_EQUAL),UM(GREATER_THAN_OR_EQUAL),      FT,      FT,       FT,       FT,      FT,      FT,       FT,                            FT,      FT,       FT,      FT,
     ...
     * `---------------------------------------------------------------------------------------------------------------------------------------------------------------------------'
     */
    [_GREEK] = LAYOUT_combined(
        FT,               FT,                 FT,                 FT,                 FT,                 FT,                FT,                FT,                 FT,                FT,               FT,                 FT,                  FT,                FT,
        FT,               FT,                 FT,                 FT,                 FT,                 FT,                FT,                FT,                 FT,                FT,               FT,                 FT,                  FT,                FT,
        FT,      FT,      FT,       FT,       FT,                 FT,       FT,       FT,       FT,       FT,       FT,      FT,       FT,      FT,       FT,       FT,       FT,      FT,      FT,      FT,       FT,       FT,                  FT,      FT,       FT,      FT,

        FT,      FT,      FT,       FT,       FT, UM(CONTOUR_INTEGRAL),  FT,  UM(COPTIC_LC_DEI), UM(DOUBLE_DAGGER), UM(NABLA), UM(CENT), UM(DEGREE), UM(APL_QUAD), UM(DIVISION), UM(MULTIPLICATION), UM(PILCROW), UM(LARGE_CIRCLE), UM(HORIZONTAL_BAR), UM(APPROXIMATELY_EQUAL_TO), UM(DOUBLE_VERTICAL_LINE), UM(SQUARE_IMAGE_OF), UM(SQUARE_ORIGINAL_OF),       FT,       FT,                 FT,       FT,      FT,
        FT,      FT,      FT,       FT,       FT,       FT,       FT,  UP(GREEK_LC_THETA, GREEK_UC_THETA), UP(GREEK_LC_OMEGA, GREEK_UC_OMEGA), UP(GREEK_LC_EPSILON, GREEK_UC_EPSILON), UP(GREEK_LC_RHO, GREEK_UC_RHO), UP(GREEK_LC_TAU, GREEK_UC_TAU),UP(GREEK_LC_PSI, GREEK_UC_PSI), UP(GREEK_LC_UPSILON, GREEK_UC_UPSILON),UP(GREEK_LC_IOTA, GREEK_UC_IOTA), UP(GREEK_LC_OMICRON, GREEK_UC_OMICRON),UP(GREEK_LC_PI, GREEK_UC_PI), UM(MATHEMATICAL_LEFT_WHITE_SQUARE_BRACKET), UM(MATHEMATICAL_RIGHT_WHITE_SQUARE_BRACKET),     FT,      FT,      FT,                            FT,      FT,       FT,      FT,
        FT,      FT,      FT,       FT,       FT,       FT,       FT,  UP(GREEK_LC_ALPHA, GREEK_UC_ALPHA), UP(GREEK_LC_SIGMA, GREEK_UC_SIGMA), UP(GREEK_LC_DELTA, GREEK_UC_DELTA), UP(GREEK_LC_PHI, GREEK_UC_PHI), UP(GREEK_LC_GAMMA, GREEK_UC_GAMMA),UP(GREEK_LC_ETA, GREEK_UC_ETA), UP(GREEK_LC_YOT, GREEK_UC_YOT),UP(GREEK_LC_KAPPA, GREEK_UC_KAPPA), UP(GREEK_LC_LAMDA, GREEK_UC_LAMDA), UM(TWO_DOT_LEADER), UM(BULLET),      FT,      FT,      FT,                                     FT,      FT,       FT,      FT,
        FT,      FT,      FT,       FT,       FT,       FT,       FT,  FT,  UP(GREEK_LC_ZETA, GREEK_UC_ZETA), UP(GREEK_LC_XI, GREEK_UC_XI), UP(GREEK_LC_CHI, GREEK_UC_CHI), UP(GREEK_LC_FINAL_SIGMA, GREEK_UC_SIGMA),UP(GREEK_LC_BETA, GREEK_UC_BETA), UP(GREEK_LC_NU, GREEK_UC_NU),UP(GREEK_LC_MU, GREEK_UC_MU), UM(MUCH_LESS_THAN),UM(MUCH_GREATER_THAN), UM(INTEGRAL),      FT,      FT,      FT,      FT,                            FT,      FT,       FT,      FT,
        FT,      FT,      FT,       FT,       FT,       FT,       FT,       FT,       FT,       FT,       FT,       FT,      FT,       FT,      FT,       FT,      FT,       FT,       FT,      FT,      FT,                                     FT,      FT,       FT,      FT,
                                                                                                          FT
    ),
...
void leader_end_user(void) {
    ...
    // Copy word
    if (leader_sequence_two_keys(KC_W, KC_C)) {
  ...
  if (record->event.pressed) {
    switch(keycode) {
      ...
      case H_LEFT_PAREN__LEFT_BRACKET:
        if (get_mods() & MODS_SHIFT_MASK) {
          CLEAN_MODS(
            send_string(SS_TAP(X_LEFT_BRACKET))
          );
        } else {
          send_string("(");
        }
      ...
      case H_RIGHT_PAREN__RIGHT_BRACKET:
        if (get_mods() & MODS_SHIFT_MASK) {
          CLEAN_MODS(
            send_string(SS_TAP(X_RIGHT_BRACKET))
          );
        } else {
          send_string(")");
        }
